...
import { FloorLevelData, defaultFloorLevels } from "@/data/floorPlansData";
import {
  Maximize2,
  Download,
  X,
...
const AccordionMark = ({ open }: { open: boolean }) => (
  <span aria-hidden="true" className="relative block h-3.5 w-3.5 shrink-0">
    ...
    <span
      className={`absolute left-1/2 top-0 h-full w-px -translate-x-1/2 bg-current transition-transform duration-300 motion-reduce:transition-none ${open ? "scale-y-0" : "scale-y-100"
        }`}
    />
  </span>
);
...
export const FloorPlansAscent: React.FC = () => {
  ...
  const rawLevels = cms?.data?.floorLevels && cms.data.floorLevels.length > 0 ? cms.data.floorLevels : defaultFloorLevels;
  const floorLevels = [...rawLevels].sort((a, b) => (a.order ?? 0) - (b.order ?? 0));
  ...
  // Sync initial sub-units. Bail out by returning `prev` when nothing actually
  // changes so this effect doesn't trigger an infinite re-render loop (the
  // `floorLevels` array is a fresh reference on every render).
  useEffect(() => {
  ...
  return (
    <div className="apartments-light-page">
      <section
        className="apartments-shell grid min-h-[calc(100svh-4.75rem)] lg:min-h-[calc(100svh-4.9rem)] lg:h-[calc(100svh-4.9rem)] lg:grid-cols-[minmax(22rem,30vw)_minmax(0,1fr)] lg:overflow-hidden"
      ...
      >
        ...
        <aside className="apartments-directory flex min-h-[42rem] flex-col lg:min-h-0">
          <header className="apartments-directory-header px-6 py-8 sm:px-10 lg:px-10 lg:py-9 xl:px-12">
            <div className="mb-4 flex items-baseline justify-between gap-5 font-sans text-xs">
              ...
              <p className="apartments-count">
                <span className="mr-1.5">{String(floorLevels.length).padStart(2, "0")}</span>
                residential tiers
              </p>
            </div>
          ...
          >
            {floorLevels.map((fl, index) => {
              const isSelected = activeFloorIndex === index;
              const isExpanded = expandedFloorIndex === index;
              ...
              return (
                <div key={fl.id} className="apartment-row">
                  <button
                    ...
                    className={`apartment-row-button group grid min-h-16 w-full grid-cols-[2.5rem_1fr_auto] items-center gap-3 text-left font-sans ${isSelected ? "is-selected" : ""
                      }`}
                  >
                    <span className="apartment-number text-xs font-semibold tabular-nums uppercase">
                      L{fl.floorCode}
                    </span>
                    <span className="text-sm font-medium">{fl.title}</span>
                  ...
                  <AnimatePresence initial={false}>
                    {isExpanded && (
                      ...
                      >
                        <div className="apartment-row-details pb-7 pl-12 pr-1">
                          <dl className="grid grid-cols-2 gap-x-6 gap-y-4 pt-4 font-sans">
                            <div>
                              <dt className="text-[0.68rem] uppercase tracking-wider text-[var(--body-ink)]">Units range</dt>
                              <dd className="mt-1 text-sm font-medium text-[var(--ink)]">{fl.unitsRange}</dd>
                            </div>
                            <div>
                              <dt className="text-[0.68rem] uppercase tracking-wider text-[var(--body-ink)]">Total area</dt>
                              <dd className="mt-1 text-sm font-medium text-[var(--ink)]">{fl.areaSqFtRange}</dd>
                            </div>
                            <div className="col-span-2">
                              <dt className="text-[0.68rem] uppercase tracking-wider text-[var(--body-ink)]">Residences</dt>
                              <dd className="mt-1 text-sm text-[var(--ink)]">{fl.residenceCount} Luxury Suites</dd>
                            </div>
                          </dl>

                          {fl.highlight && (
                            <div className="mt-4 border-l-2 border-[var(--brand-taupe)] pl-3 py-1">
                              <p className="font-serif italic text-sm text-[var(--brand-taupe-deep)]">
                                &ldquo;{fl.highlight}&rdquo;
                              </p>
                            </div>
                          )}

                          <p className="apartment-description mt-4 font-sans text-xs leading-relaxed text-[var(--body-ink)]">
                            {fl.description}
                          </p>

                          <a
                            href="#floor-plan-stage"
                            className="apartment-mobile-media-link mt-4 inline-flex items-center gap-2 font-sans text-xs lg:hidden"
                          >
                            View floor plan layout <span aria-hidden="true">↓</span>
                          </a>

                          {/* Sub-unit Tabs */}
                          {fl.subUnits && fl.subUnits.length > 0 && (
                            <div className="mt-6" role="tablist" aria-label={`Sub-units in ${fl.title}`}>
                              ...
                              {fl.subUnits.map((sub, sIdx) => {
                                ...
                                return (
                                  <button
                                    ...
                                    className={`apartment-view-tab flex min-h-11 w-full items-center justify-between text-left font-sans text-xs ${subSelected ? "is-active font-medium" : ""
                                      }`}
                                  >
                                    <span>{sub.name} ({sub.bedrooms})</span>
                                    <span className="tabular-nums opacity-60">0{sIdx + 1}</span>
                                  </button>
                                );
                              })}
                            </div>
        ...
        <div
          id="floor-plan-stage"
          className="apartments-media relative flex min-h-[68svh] flex-col justify-between overflow-hidden p-6 sm:p-10 lg:min-h-0 xl:p-12 bg-[var(--paper)]"
        >
          {/* Top Bar Actions */}
          <div className="z-20 flex items-center justify-between gap-4">
            ...
            <div className="flex items-center gap-3">
              ...
              >
                <span>Floor Plans PDF</span> <DownloadIcon />
              </a>
          ...
          <div className="relative my-auto flex-1 min-h-0 w-full h-full flex items-center justify-center py-3">
                ...
                <img
                  ...
                  onError={(e) => {
                    (e.target as HTMLImageElement).src =
                      "https://meshoiyjbcacrlvdaoov.supabase.co/storage/v1/object/public/bolsover-media/floor-plans/ground/master-plan.png";
                  }}
                  className="h-full w-full max-h-[calc(100vh-14rem)] lg:max-h-[calc(100vh-11.5rem)] object-contain transition-transform duration-300 group-hover:scale-[1.02] drop-shadow-lg"
                />
          ...
          <div className="z-20 flex items-end justify-between gap-6 border-t border-[var(--line)] pt-5 text-[var(--ink)]">
            <div>
              <p className="font-serif text-2xl sm:text-3xl text-[var(--ink)] font-normal">{activeFloor.title}</p>
              <p className="mt-1 font-serif italic text-sm text-[var(--body-ink)]">
                {activeFloor.subtitle || activeFloor.highlight}
              </p>
            </div>
            <div className="text-right font-sans text-xs">
              <span className="block font-medium text-[var(--brand-taupe-deep)]">{activeFloor.areaSqFtRange}</span>
              <span className="text-[var(--body-ink)]">{activeFloor.unitsRange}</span>
            </div>
          </div>
              ...
              {[...floorLevels].reverse().map((fl) => {
                ...
                return (
                  <button
                    ...
                    className={`relative flex h-7 w-7 items-center justify-center rounded-full text-[10px] font-sans transition-all duration-200 ${isSelected
        ...
        {inspectingPlan && (
          ...
          >
            ...
            <div
              ...
              className={`relative my-auto flex h-[76vh] w-full max-w-[92vw] items-center justify-center overflow-hidden rounded-2xl ${zoomScale > 1 ? (isDragging ? "cursor-grabbing" : "cursor-grab") : "cursor-default"
                }`}
            >
              <motion.div
                animate={{
